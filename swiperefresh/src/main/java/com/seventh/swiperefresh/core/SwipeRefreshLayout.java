package com.seventh.swiperefresh.core;
...
import com.seventh.swiperefresh.R;
...
public class SwipeRefreshLayout extends FrameLayout implements NestedScrollingParent {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        int resId;
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.SwipeRefreshLayout);
        Resources resources = context.getResources();

        //Indicator背景颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLayout_guidance_view_bg_color, -1);
        if (resId == -1) {
            mGuidanceViewBgColor = ta.getColor(R.styleable.SwipeRefreshLayout_guidance_view_bg_color,
                    Color.WHITE);
        } else {
        ...
        //加载文字颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLayout_guidance_text_color, -1);
        if (resId == -1) {
            mGuidanceViewTextColor = ta.getColor(R.styleable.SwipeRefreshLayout_guidance_text_color,
                    Color.BLACK);
        } else {
        ...
        //进度条背景颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLayout_progress_bg_color, -1);
        if (resId == -1) {
            mProgressBgColor = ta.getColor(R.styleable.SwipeRefreshLayout_progress_bg_color,
                    Color.WHITE);
        } else {
        ...
        //进度条颜色
        resId = ta.getResourceId(R.styleable.SwipeRefreshLayout_progress_bar_color, -1);
        if (resId == -1) {
            mProgressColor = ta.getColor(R.styleable.SwipeRefreshLayout_progress_bar_color,
                    Color.RED);
        } else {
        ...
        //下拉刷新文字描述
        resId = ta.getResourceId(R.styleable.SwipeRefreshLayout_refresh_text, -1);
        if (resId == -1) {
            mRefreshDefaulText = ta.getString(R.styleable.SwipeRefreshLayout_refresh_text);
        } else {
            mRefreshDefaulText = resources.getString(resId);
        }

        //上拉加载文字描述
        resId = ta.getResourceId(R.styleable.SwipeRefreshLayout_load_text, -1);
        if (resId == -1) {
            mLoadDefaulText = ta.getString(R.styleable.SwipeRefreshLayout_load_text);
        } else {
